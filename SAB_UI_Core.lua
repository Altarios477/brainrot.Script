...
createButton("👥 ESP Игроки", function() for _, player in pairs(Players:GetPlayers()) do if player ~= LocalPlayer then player.CharacterAdded:Connect(function(char) local root = char:WaitForChild("HumanoidRootPart", 5) if root and not root:FindFirstChild("ESP") then local gui = Instance.new("BillboardGui", root) gui.Name = "ESP" gui.AlwaysOnTop = true gui.Size = UDim2.new(0, 110, 0, 35) gui.StudsOffset = Vector3.new(0, 2.5, 0)
