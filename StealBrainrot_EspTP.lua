...
local function up()
	...
	if root then root.CFrame += Vector3.new(0, 240, 0) end
end
...
for _, obj in ipairs(workspace:GetDescendants()) do
	if obj:IsA("TextLabel") or obj:IsA("TextBox") then
		if string.find(obj.Text, "%d+s") then
			local parent = obj:FindFirstAncestorOfClass("Model") or obj:FindFirstAncestorWhichIsA("BasePart")
			if parent then
