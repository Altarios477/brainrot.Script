...
-- Плавный телепорт с обходом античита
local function safeTeleport(targetCFrame)
    local steps = 10
    local current = humanoidRootPart.CFrame
    local step = (targetCFrame.Position - current.Position) / steps
    
    for i = 1, steps do
        if not humanoidRootPart or not humanoidRootPart.Parent then break end
        
        local newPos = current.Position + step
        humanoidRootPart.CFrame = CFrame.new(newPos) * current.Rotation
...
UpButton.MouseButton1Click:Connect(function()
    ...
    safeTeleport(CFrame.new(newPos))
end)
